...
import { streamMessage, estimateCost } from '@/lib/ai/provider'
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // streamMessage routes to the correct provider automatically
        await streamMessage({
        ...
        const { data: savedMsg } = await supabase
          ...
          .insert({
            ...
            metadata: {
              ...
              context_messages: contextResult.included_count,
            },
          })
          .select().single()
        ...
        await Promise.all([
          ...
          logActivity({
            ...
            metadata: { worker_name: worker.name, model: worker.model, provider: worker.provider, tokens: inputTokens + outputTokens, cost },
          }),
        ])
